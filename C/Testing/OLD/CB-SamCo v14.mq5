...
input    bool         printDebug    =  false;   //Print DeBuging Info?
...
string         indicatorName              =  "CB-SamCo v12";
   ...
   for(int i = limit - 1; i >= 0; i--)
     {
      ...
      int      hourlyBar  = iBarShift(Symbol(), PERIOD_H1, time[i], false);
      datetime hourlyTime = iTime(Symbol(), PERIOD_H1, hourlyBar);
      ...
      int      copyDailyFastBuffer    =   CopyBuffer(dailyFastHandle, 0, dailyTime, 1, dailyFastArray);
      int      copyDailySlowBuffer    =   CopyBuffer(dailySlowHandle, 0, dailyTime, 1, dailySlowArray);
      int      copyHourlyFastBuffer   =   CopyBuffer(hourlyFastHandle, 0, hourlyTime, 1, hourlyFastArray);
      int      copyHourlySlowBuffer   =   CopyBuffer(hourlySlowHandle, 0, hourlyTime, 1, hourlySlowArray);
