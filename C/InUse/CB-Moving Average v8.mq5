...
//+------------------------------------------------------------------+
 ...
 int maxLookback = (TimeFrameToMinutes(PERIOD_H1) *200)  / TimeFrameToMinutes(PERIOD_CURRENT); //800
   int startCount = 1;
   int limit;

   (prev_calculated == 0) ? (limit = rates_total - maxLookback) : (limit = rates_total - prev_calculated + startCount);
...
//+------------------------------------------------------------------+
    ...
    for(int i = startCount; i < limit; i++)
     {
